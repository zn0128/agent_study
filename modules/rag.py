...
def search_knowledge_base(query: str) -> dict:
    ...
    hits = [
        ...
        if any(k in q for k in item['kw'])
        or item['topic'].lower().replace('（', '').replace('）', '').replace('(', '').replace(')', '') in q
    ][:3]  # 最多返回 3 个片段，避免 Context 过长
    ...
    return {
        'found':   True,
        'count':   len(hits),
        'results': [{'topic': h['topic'], 'content': h['content']} for h in hits],
    }
