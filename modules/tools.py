...
TOOL_DEFINITIONS = [
    ...
    },
    {
        ...
        'description': (
            '将复杂任务分解给多个 SubAgent 并行执行，每个 SubAgent 是一次独立的 Claude API 调用。'
            '适用场景：任务需要从多个角度分析、需要分阶段完成、需要整合多方面内容（如写完整报告、做深度研究）。'
            '普通问答、单一查询、简单计算不需要使用此工具。'
        ),
        'input_schema': {
            'type':       'object',
            'properties': {
                'task': {'type': 'string', 'description': '需要分解执行的完整任务描述'},
            },
            'required': ['task'],
        },
    },
...
def execute_tool(name: str, inp: dict, session: dict, add_memory: Callable, memory_bank=None) -> dict:
    ...
    # ── 工具 4：保存记忆（联动 Memory 模块）───────
        ...
        # 只写入 Long-term Memory（磁盘 + 向量索引）
        # Short-term 由 auto_extract_session 自动维护，不在这里越界写入
        add_memory(entry)
