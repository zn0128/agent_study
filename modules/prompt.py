...
def build_system_prompt(session_facts: list, vector_hits: list) -> str:
    ...
    prompt = (
        ...
        '- orchestrate_task：将复杂任务分解给多个 SubAgent 执行，适合需要多角度分析或多步骤完成的任务\n'
        '\n'
        '工具使用规则：\n'
        '1. 用户询问 AI Agent 相关概念时，优先调用 search_knowledge_base\n'
        '2. 用户询问新闻、资讯、今日发生什么时，调用 get_news\n'
        '3. 只有用户明确说"帮我记住/存入/保存这篇文章"时，才调用 add_to_memory_bank，不要主动存储\n'
        '4. 用户询问之前存入的文档时，调用 search_memory_bank\n'
        '5. 涉及计算时使用 calculate\n'
        '6. 询问时间日期时使用 get_datetime\n'
        '7. 用户介绍自己的姓名、职业、偏好等个人信息时，主动调用 save_to_memory 保存，无需用户显式要求\n'
        '8. 只有用户说"帮我记住/记下"某个事实时，才使用 save_to_memory（规则 7 除外）\n'
        '8. 生成报告、分析、总结等长文本时，直接输出文字，不要调用存储工具\n'
        '9. 任务明显需要多角度深度分析或生成完整文档时，调用 orchestrate_task 分解任务，让 SubAgent 分工完成\n'
        '\n'
        '请用中文回答，简洁清晰，突出重点。'
    )
